/*
 * libgpiod LED Blink Example (v1 API)
 *
 * This example demonstrates basic GPIO output using libgpiod v1.x API.
 * It blinks an LED connected to GPIO 23 (BCM numbering).
 *
 * Hardware:
 * - LED with 220Ω resistor connected to GPIO 23 (Physical pin 16)
 * - GND connection (any ground pin)
 *
 * libgpiod is the official Linux kernel GPIO library that works across
 * all Linux ARM SBCs (Raspberry Pi, Orange Pi, Rock Pi, etc.)
 */

#include <gpiod.h>
#include <stdio.h>
#include <unistd.h>

#define GPIO_CHIP "gpiochip0"
#define LED_PIN 23
#define BLINK_COUNT 10

int main(void) {
    struct gpiod_chip *chip;
    struct gpiod_line *line;
    int ret;

    printf("libgpiod v1 LED Blink Example\n");
    printf("==============================\n\n");

    // Open GPIO chip
    chip = gpiod_chip_open_by_name(GPIO_CHIP);
    if (!chip) {
        perror("Failed to open GPIO chip");
        return 1;
    }

    printf("Opened GPIO chip: %s\n", GPIO_CHIP);
    printf("GPIO chip label: %s\n", gpiod_chip_label(chip));
    printf("Number of lines: %u\n\n", gpiod_chip_num_lines(chip));

    // Get GPIO line
    line = gpiod_chip_get_line(chip, LED_PIN);
    if (!line) {
        perror("Failed to get GPIO line");
        gpiod_chip_close(chip);
        return 1;
    }

    printf("GPIO line %d: %s\n", LED_PIN, gpiod_line_name(line));

    // Request line as output with initial value LOW
    ret = gpiod_line_request_output(line, "blink-example", 0);
    if (ret < 0) {
        perror("Failed to request GPIO line as output");
        gpiod_chip_close(chip);
        return 1;
    }

    printf("Configured GPIO %d as output\n", LED_PIN);
    printf("Blinking LED %d times...\n\n", BLINK_COUNT);

    // Blink LED
    for (int i = 0; i < BLINK_COUNT; i++) {
        // Turn LED ON
        ret = gpiod_line_set_value(line, 1);
        if (ret < 0) {
            perror("Failed to set GPIO value");
            break;
        }
        printf("LED ON  (iteration %d/%d)\n", i + 1, BLINK_COUNT);
        sleep(1);

        // Turn LED OFF
        ret = gpiod_line_set_value(line, 0);
        if (ret < 0) {
            perror("Failed to set GPIO value");
            break;
        }
        printf("LED OFF (iteration %d/%d)\n", i + 1, BLINK_COUNT);
        sleep(1);
    }

    printf("\nBlink complete!\n");

    // Cleanup (automatic on exit, but explicit is good practice)
    gpiod_line_release(line);
    gpiod_chip_close(chip);

    printf("GPIO resources released\n");

    /*
     * NOTE: GPIO State Persistence
     *
     * By default, the GPIO line may revert to its default state (usually input)
     * when this program exits and the line is released.
     *
     * On Raspberry Pi, to maintain the GPIO state after program exit, add this
     * line to /boot/firmware/config.txt (or /boot/config.txt on older systems):
     *
     *     dtparam=strict_gpiod
     *
     * Then reboot. This enables persistent GPIO state across process restarts.
     *
     * See docs/LIBGPIOD_SETUP.md for more details.
     */

    return 0;
}
